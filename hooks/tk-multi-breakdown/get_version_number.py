...
import sys, os
sys.path.append('X:/dev/ss_dev_harrisonforbes/MG2_repo/shotgun')
from api3 import utils_api3
...
class GetVersionNumber(HookBaseClass):
    ...
    def execute(self, template, curr_fields, **kwargs):
        ...
        highest_version = 0
        proj = os.environ['CURR_PROJECT']
        sg_inst = utils_api3.ShotgunApi3()

        latest_file = sg_inst.get_latest_asset_pub_in_proj_by_step(proj, curr_fields['Asset'], curr_fields['Step'])

        highest_version = latest_file.split('.')[-2]
