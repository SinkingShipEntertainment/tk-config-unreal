...
class BasicFilePublishPlugin(HookBaseClass):
    ...
    subclasses prior to calling methods on the base class::
        ...
        ``sequence_paths`` - If set in the item properties dictionary, implies
            ...
            files on disk matching the "path". If the ``work_template``
            property is set, and corresponds to the listed frames, fields will
            be extracted and applied to the publish_template (if set) and
            copied to that publish location.
    ...
    :meth:`Item.properties` or :meth:`Item.local_properties`.
        ...
        publish_kwargs - If set, will be used to update the dictionary of
            kwargs passed to :meth:`tank.util.register_publish`. Because this
    ...
    This plugin will also set an ``sg_publish_data`` property on the item
    during the ``publish`` method which may be useful for child items.
    ...
    def description(self):
        ...
        url = "https://support.shotgunsoftware.com/hc/en-us/articles/219033078"
        ...
        Shotgun will also reflect the version number identified in the
        filename. The basic workflow recognizes the following version formats
        by default:
    ...
    def validate(self, settings, item):
        ...
        if publishes:
            ...
            if "work_template" in item.properties or publish_template:
                ...
                # happening.
                error_msg = (
                    "Can not validate file path. There is already a publish "
                    "in Shotgun that matches this path. Please uncheck this "
    ...
    def publish(self, settings, item):
        ...
        publish_path = self.get_publish_path(settings, item)
        publish_dependencies_paths = self.get_publish_dependencies(
            settings,
            item
        )
        publish_user = self.get_publish_user(settings, item)
        publish_fields = self.get_publish_fields(settings, item)
        # catch-all for any extra kwargs that should be passed to
        # register_publish.
        publish_kwargs = self.get_publish_kwargs(settings, item)

        # if the parent item has publish data, get it id to include it in the
        ...
        self.logger.debug(
            ...
            extra={
                "action_show_more_info": {
                    ...
                    "tooltip": "Show the complete Shotgun Publish Entity dict",
                    "text": "<pre>%s</pre>"
    ...
    def finalize(self, settings, item):
        ...
        info_m = "Cleared the status of all previous, conflicting publishes"
        self.logger.info(info_m)
    ...
    def get_publish_version(self, settings, item):
        ...
        if work_template:
            if work_template.validate(path):
                debug_m = "Work file template configured and matches file."
                self.logger.debug(debug_m)
                work_fields = work_template.get_fields(path)
        ...
        if work_fields:
            ...
            if "version" in work_fields:
                ...
                debug_m = "Retrieved version number via work file template."
                self.logger.debug(debug_m)

        else:
            debug_m = "Using path info hook to determine publish version."
            self.logger.debug(debug_m)
    ...
    def _get_next_version_info(self, path, item):
        ...
        If templates are configured, use template logic. Otherwise, fall back
        to the zero configuration, path_info hook logic.
        ...
        # if we have template and fields, use them to determine the version
        # info
        if work_fields and "version" in work_fields:

            # template matched. bump version number and re-apply to the
            # template
    ...
    def _save_to_next_version(self, path, item, save_callback):
        ...
        available version on disk. If a version can not be detected in the
    ...
    ###########################################################################
    # SSE QCTool realted methods - DW 2021-01-21
