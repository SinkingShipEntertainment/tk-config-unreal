...
class MayaASSPublishPlugin(HookBaseClass):
    """
    Plugin for publishing an open maya session as an exported ASS.
    ...
    def description(self):
        ...
        return """
        <p>This plugin publishes session geometry as an Arnold Standin for the
        current session. Any session geometry under the '|master|model'
        hierarchy will be exported to the path defined by this plugin's
        configured "Publish Template" setting. The plugin will fail to validate
        if the "mtoa" plugin is not enabled or cannot be found, or if there is
        no '|master|model' hierarchy found.</p>
        """
    ...
    def settings(self):
        ...
        base_settings = super(MayaASSPublishPlugin, self).settings or {}
    ...
    def validate(self, settings, item):
        ...
        # SSE: check that the |master|model hierarchy exists (DW 2020-07-28)
        m_node = '|master|model'
        if not cmds.objExists(m_node):
            error_msg = 'Validation failed because there is no '
            error_msg += '"{}" node hierarchy in the scene.'.format(m_node)
            self.logger.error(error_msg)
            raise Exception(error_msg)
        ...
        item.properties["publish_type"] = "Maya ASS"
        ...
        return super(MayaASSPublishPlugin, self).validate(settings, item)
    ...
    def publish(self, settings, item):
        ...
        super(MayaASSPublishPlugin, self).publish(settings, item)
