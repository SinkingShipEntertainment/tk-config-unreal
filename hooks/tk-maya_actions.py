...
class MayaActions(HookBaseClass):
    ...
    def _create_reference(self, path, sg_publish_data):
        ...
        # --- (DW 2019-03-22)...
        #versionless_filepath = path.replace('\\', '/')
        #if '/RIG/' in versionless_filepath:
        #    versionless_filepath = versionless_filepath.replace('/maya/', '/maya/versionless/')
        #    v_bits = versionless_filepath.split('.')
        #    versionless_filepath = '.'.join([v_bits[0], v_bits[2]])
        #
        #    if os.path.isfile(versionless_filepath):
        #        app.log_debug('RIG: versionless_filepath >> {}'.format(versionless_filepath))
        #        path = versionless_filepath
        # ---^^^
