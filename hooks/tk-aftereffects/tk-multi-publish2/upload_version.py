...
class AfterEffectsUploadVersionPlugin(HookBaseClass):
    ...
    """
    # SSE - added from Engine for sse_get_render_files
    # TODO - Remove this hook if SG updates Engine to query the startframe.
    ...
    def publish(self, settings, item):
        ...
        # SSE - log the publish_data for debugging
        self.logger.debug(
            "Populated Publish data...",
            extra={
                "action_show_more_info": {
                    "label": "SSE Publish Data",
                    "tooltip": "Show the complete Publish data dictionary",
                    "text": "<pre>%s</pre>" % (
                        pprint.pformat(publish_data),)
                }
            }
        )
        # SSE - log the Rendering_data for debugging
        self.logger.debug(
            "Populated Rendering data...",
            extra={
                "action_show_more_info": {
                    "label": "SSE Rendering Data",
                    "tooltip": "Show the complete Rendering data dictionary",
                    "text": "<pre>%s</pre>" % (
                        pprint.pformat(rendering_data),)
                }
            }
        )
        ...
        # SSE - strip out .%4d.exr to publish a clean version name
        old_code = version_data['code']
        version_data['code'] = '.'.join([i for i in old_code.split('.')[:-2]])
        version_data['sg_status_list'] = 'arev'
        # SSE - log the new Version data for debugging
        self.logger.debug(
            "Populated SSE Version data...",
            extra={
                "action_show_more_info": {
                    "label": "SSE Version Data",
                    "tooltip": "Show the complete Version data dictionary",
                    "text": "<pre>%s</pre>" % (
                        pprint.pformat(version_data),)
                }
    ...
    def __render_movie_from_sequence(self, sequence_path, queue_item, mov_output_module_template):
        ...
        # SSE - switched to use function in this hook, instead of engine, to get the correct first frame#
        #for first_frame, _ in self.parent.engine.get_render_files(sequence_path, queue_item):
        for first_frame, _ in self.sse_get_render_files(sequence_path, queue_item):
            break
    ...
    # SSE
    # Function repurposed from {SGTK_PATH}\install\app_store\tk-aftereffects\v0.1.4\engine
    # Engine function used queue_item.timeSpanStart to return start_time in seconds insteaf of the correct frame#
    # If comp starts at a non-zero frame, f.e. frame-1, engine returns 0.
    # TODO @Shervin - submitted ticket to SG with my solution. IF they make updates to AE engine, we can remove this hook.
    # TODO @Shervin - cleanup the excessive self.logger.debug lines!
    def sse_get_render_files(self, path, queue_item):
        """
        ...
        # SSE - use displayStartTime to get frame# instead of timeSpanStart that returns seconds
        #start_time = int(round(queue_item.timeSpanStart / frame_time, 3))
        start_time = int(round(queue_item.comp.displayStartTime / frame_time, 3))
    ...
    def __get_additional_version_data(self, queue_item, path_to_frames):
        ...
        out_dict = {}
        frame_numbers = []
        # SSE - Use function in the hook, instead of engine, to get the correct first frame#
        #for _, fn in self.parent.engine.get_render_files(path_to_frames, queue_item):
        for _, fn in self.sse_get_render_files(path_to_frames, queue_item):
