...
class AfterEffectsCopyRenderPlugin(HookBaseClass):
    ...
    # SSE - added from Engine used in sse_get_render_files
    ...
    def __iter_publishable_paths(
        ...
        for each_path in render_paths:
            ...
            target_path = None
            # SSE - use function in this hook, instead of engine, to get the correct first frame #
            #for file_path, frame_no in self.parent.engine.get_render_files(each_path, queue_item):
            for file_path, frame_no in self.sse_get_render_files(each_path, queue_item):
    ...
    # TODO @Shervin - submitted ticket to SG with my solution. If/when they make updates to AE engine, we can remove this hook.
    ...
    def sse_get_render_files(self, path, queue_item):
        ...
        self.logger.debug(">> Comp start_time = {}".format(start_time))
